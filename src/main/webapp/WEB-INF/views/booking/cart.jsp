					...
					<div class="container">
						...
						<div class="row">
							...
							<div class="col-6 col-md-6">
								...
								<input type="hidden" name="roomquan" value="${roomquan }">
							</div>
						</div>
						<hr>
						<div class="row">
						...
						<div class="row">
							...
							<div class="col-6 col-md-6">
								<small style="color: green;">- ${room.copponRoom.sale} %</small>
							</div>
						</div>
						<div class="row">
							<div class="col-6 col-md-6">
							...
							<div class="col-6 col-md-6">
								<small style="color: green;" >-<span id="resultSale"> 0</span> % </small>
							</div>
						</div>
						<hr>
						<div class="row">
							...
							<div class="col-6 col-md-6">
								<h4 style="color: #2E64FE;">$<span id="finalfrice"> ${(room.price * days * roomquan)  * (100 - (room.copponRoom.sale + room.hotel.copponHotel.sale) )/100 }</span></h4>
								<input type="hidden"  id="finalTemp" value=" ${(room.price * days * roomquan)  * (100 - (room.copponRoom.sale + room.hotel.copponHotel.sale) )/100 }" />
								<p>(2 night)</p>
							</div>
						</div>

						<div class="input-group mb-3">
							<input type="text" class="form-control" id="valuecode" name="giftcode" placeholder="Gift code">
			...
			<div class="card">
						...
						<div class="row">
							<div class="col-10">
								<label for="inputPassword5">Contact's name <span
									style="color: red">*</span></label> <input type="text" value="${user.fullname }"
									id="nameuser" class="form-control" name="namestaying"
									aria-describedby="nameHelpBlock">
									<input type="text" value="${user.id }" name="acid">
									
									 <small
									id="nameHelpBlock" class="form-text text-muted"> Name of the customer staying
									
								</small>
							</div>
							<div class="col-10">
								<label for="inputPassword5">Email <span
									style="color: red">*</span></label> <input type="email" value="${user.email }" name="email"
									id="inputEmail" class="form-control"
									aria-describedby="emailHelpBlock"> <small
									id="inputEmail" class="form-text text-muted">
							...
							<p
									>
									Support payment at the hotel !
								 </p>
							</div>
						
							<div class="col-4">
								<input type="submit" value="Booking" class="btn-s"> <input type="hidden" value="payathotel" name="paytype">
							</div>
						</div>
				...
				<form method="post" action="${pageContext.request.contextPath }user/pay">
					<div class="row">
						...
						<input type="hidden" value="${checkout }" name="checkout">
						<input type="hidden" name="roomquan" value="${roomquan }">
						<input type="hidden"  id="valuecode2" name="giftcode" >
						<input type="hidden" value="${user.id }" name="acid">
						 <input type="hidden" value="${user.fullname }"
									id="nameuser2" name="namestaying"
									>
									<input type="hidden" value="${user.email }" name="email" id="inputEmail2" >
									<input type="hidden" value="" name="note" id="note2">
						
								<input type="hidden" id="pay" value="${(room.price * days * roomquan)  * (100 - (room.copponRoom.sale + room.hotel.copponHotel.sale) )/100 }" name="price" />
	<button type="submit" class="btn-s"> Payment with Paypal </button>
							</div>
						</div>
										...
										function() {
											...
											$
													.ajax({
														...
														if(result.id == null){
															$('#resFail').text('No discount code found');
															}else{
																alert('Congratulations !  - ' + result.sales +'%' );
																
																$('#resultSale').text( result.sales );
																var priceTemp = $('#finalTemp').val();
																
																$('#finalfrice').text( priceTemp * (100 -result.sales )/100 );
																$('#pay').val( priceTemp * (100 -result.sales )/100 );
																$('#valuecode2').val($('#valuecode').val());
